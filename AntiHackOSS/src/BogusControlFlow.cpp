...
struct BogusControlFlow : public FunctionPass {
  ...
  std::string homeDir;
  bool doBogusControlFlow;
  llvm::raw_fd_ostream *logFile;
  std::set<std::string> bcfedFuncs;
  int ObfTimes = 0;
  int ObfProbRate = 0;

  BogusControlFlow() : FunctionPass(ID) {}
  BogusControlFlow(bool flag) : FunctionPass(ID) {
    this->doBogusControlFlow = flag;
    BogusControlFlow();
  }

  BogusControlFlow(llvm::json::Value* configJson, llvm::raw_fd_ostream *logFile) : FunctionPass(ID), configJson(0) 
  { 
    this->doBogusControlFlow = true;
  ...
  virtual bool runOnFunction(Function &F) {
    ...
    if (jsonObj->getArray("flatten")) {
      ...
      llvm::json::Array *BogusArray = jsonObj->getArray("flatten");
      for (auto &obj : *BogusArray) {
        ...
        if (!obj.getAsObject()->getInteger("bcf").getValueOr(1)) {
          (*logFile) << "[Frontend]: Config Error: missing 'bcf' valuein flatten array \n";
          continue;
        }
        ...
        if (ObfTimes <= 0) {
          (*logFile) << "BogusControlFlow application number bcf_loop=x must be x > 0  bcf_loop = " << ObfTimes << "\n";
        ...
        if (!((ObfProbRate > 0) && (ObfProbRate <= 100))) {
          (*logFile) << "BogusControlFlow application basic blocks percentage "
                    "bcf_prob=x must be 0 < x <= 100 ObfProbRate = " << ObfProbRate << "\n";
        ...
        if (reFuncName.match(F.getName()) && bcfedFuncs.find(F.getName().str()) == bcfedFuncs.end() ) {
          bcfedFuncs.insert(F.getName().str());
          (*logFile) << "[Frontend]: bogusControlFlow src name = "<< funcName << " target name = " << F.getName() << "\n";
          bogus(F, seed);
          doF(*F.getParent());
          (*logFile) << "[Frontend]: Successfully bogusControlFlow func " << F.getName() << "\n";
          logFile->flush();
        }
      }
    }
  ...
  void bogus(Function &F, std::string seed) {
    // SCRAMBLER
    if (seed.size() == 0) {
    ...
    (*logFile)  << "bcf: Probability rate: " << ObfProbRate << "ObfTimes = " << ObfTimes <<"\n";
    ...
    do {
      ...
      if (hasBeenModified) { // if the function has been modified
        ...
        (*logFile) << "bcf: Function's not been modified \n";
      }
      firstTime = false;
    } while (--NumObfTimes > 0);
  }
...
Pass *llvm::createBogus(llvm::json::Value* configJson, llvm::raw_fd_ostream *logFile, std::string homeDir) {
  return new BogusControlFlow(configJson, logFile);
}
